...
routerField.get('/:idusuario', async (req: Request, res: Response) => {
  ...
  try {
    const result = await conection.query('SELECT * FROM field WHERE idUsuario =?;',[idusuario])
    return res.status(200).json(result)
  } catch (error) {
    await conection.end()
    res.status(500).json({ error: error })
...
routerField.post('/', async (req: Request, res: Response) => {
  ...
  try {
    await conection.query(
      `INSERT INTO field (  name, ubicacion, idusuario ) VALUES (?,?,?);`,
      [name, ubicacion, idusuario]
    )
